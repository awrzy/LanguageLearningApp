    ...
    public partial class QuizControlFr1 : UserControl
    {
        ...
        private void InitializeQuestions()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT ID, QuestionText, CorrectAnswerIndex FROM QuestionsFr", conn))
                ...
                foreach (var (id, text, correctAnswerIndex) in loadedQuestions)
                {
                    ...
                    using (SqlCommand cmdAnswers = new SqlCommand("SELECT AnswerText, ImagePath FROM AnswersFr WHERE QuestionId = @id ORDER BY QuestionID", conn))
                    {
